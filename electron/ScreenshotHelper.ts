...
export class ScreenshotHelper {
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(tempFile)) {
          ...
          if (!buffer || buffer.length === 0) {
            throw new Error("PowerShell screenshot file is empty (0 bytes)");
          }
        ...
        console.log("Created placeholder image as fallback. Returning fallback buffer.");
  ...
  public async uploadScreenshot(
    ...
    try {
      ...
      // Determine target directory based on view
      const targetDir =
      ...
      if (this.view === "queue") {
        this.screenshotQueue.push(newFilePath);

        // Handle max screenshots limit
        if (this.screenshotQueue.length > this.MAX_SCREENSHOTS) {
          const removedPath = this.screenshotQueue.shift();
          if (removedPath) {
            try {
        ...
        this.extraScreenshotQueue.push(newFilePath);
        ...
        if (this.extraScreenshotQueue.length > this.MAX_SCREENSHOTS) {
          ...
          if (removedPath) {
            try {
              await fs.promises.unlink(removedPath);
              console.log("Removed old extra screenshot from queue:", removedPath);
            } catch (error) {
