...
const electronAPI = {
  ...
  toggleInvisibilityMode: () => ipcRenderer.invoke("toggle-invisibility-mode"),
  getInvisibilityMode: () => ipcRenderer.invoke("get-invisibility-mode"),
  toggleMousePassthroughMode: () =>
    ipcRenderer.invoke("toggle-mouse-passthrough-mode"),
  getMousePassthroughMode: () =>
    ipcRenderer.invoke("get-mouse-passthrough-mode"),
  askInterviewerQuestion: (payload: {
    question: string
    solutionCode?: string
    thoughts?: string[]
  }) => ipcRenderer.invoke("ask-interviewer-question", payload),
  triggerScreenshot: () => ipcRenderer.invoke("trigger-screenshot"),
  triggerProcessScreenshots: () =>
    ipcRenderer.invoke("trigger-process-screenshots"),
  triggerProcessTextQuery: (query: string) =>
    ipcRenderer.invoke("trigger-process-text-query", query),
  ...
  // Local model configuration methods
  ...
  onInvisibilityModeChanged: (callback: (enabled: boolean) => void) => {
  ...
  onMousePassthroughModeChanged: (callback: (enabled: boolean) => void) => {
    const subscription = (_: any, enabled: boolean) => callback(enabled)
    ipcRenderer.on("mouse-passthrough-mode-changed", subscription)
    return () => {
      ipcRenderer.removeListener("mouse-passthrough-mode-changed", subscription)
    }
  },
