...
export interface IProcessingHelperDeps {
  ...
  getLatestSolution: () => any
  setLatestSolution: (solution: any) => void
...
export interface IShortcutsHelperDeps {
  ...
  toggleInvisibilityMode: () => boolean
  toggleMousePassthroughMode: () => boolean
  moveWindowLeft: () => void
...
export interface IIpcHandlerDeps {
  ...
  clearQueues: () => void
  setView: (view: "queue" | "solutions" | "debug") => void
  getInvisibilityMode: () => boolean
  getMousePassthroughMode: () => boolean
  moveWindowLeft: () => void
  moveWindowRight: () => void
  moveWindowUp: () => void
  moveWindowDown: () => void
...
async function createWindow(): Promise<void> {
  ...
  applyInvisibilityMode(configHelper.isInvisibilityEnabled())
  ...
  if (savedOpacity <= 0.1) {
    ...
    applyMousePassthroughMode(configHelper.isMousePassthroughEnabled());
    state.isWindowVisible = true;
  }
}
...
function toggleInvisibilityMode(): boolean {
  const enabled = !configHelper.isInvisibilityEnabled()
  configHelper.setInvisibilityEnabled(enabled)
  applyInvisibilityMode(enabled)
  return enabled
}

function getInvisibilityMode(): boolean {
  return configHelper.isInvisibilityEnabled()
}

function toggleMousePassthroughMode(): boolean {
  const enabled = !configHelper.isMousePassthroughEnabled()
  configHelper.setMousePassthroughEnabled(enabled)
  ...
  applyMousePassthroughMode(enabled)
  return enabled
}

function getMousePassthroughMode(): boolean {
  return configHelper.isMousePassthroughEnabled()
}
...
function getLatestSolution(): any {
  return state.latestSolution
}

function setLatestSolution(solution: any): void {
  state.latestSolution = solution
}
...
async function uploadScreenshot(
  ...
  return (
    state.screenshotHelper?.uploadScreenshot(filePath) || {
      success: false,
      error: "Screenshot helper not initialized"
    }
  )
